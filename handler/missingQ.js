...
function questionHasKeyword(q, k, raw) {
  if (!k) return false;
  if (hasWholePhrase(q, k) || hasWholePhrase(q, k.replace(/^ال/, ""))) return true;
...
function detectKeywordInQuestion(q, raw) {
  ...
  for (const k of Object.keys(raw)) {
    if (questionHasKeyword(q, k, raw)) {
      const variantHit = [k, ...(raw[k].variants || [])]
        .find((v) => hasWholePhrase(q, v));
      hits.push({ keyword: k, variantMatched: variantHit, len: variantHit.length });
    }
  }
  /* احتفظ بالأطول فقط */
...
function extractFromContext(part, allText, kwRaw) {
  ...
  for (const [kw, data] of Object.entries(kwRaw)) {
    ...
    const variantHit = [kw, ...(data.variants || [])]
      .find((v) => hasWholePhrase(part, v));
...
function extractAllIntents(txt, intRaw) {
  ...
  for (const [intent, obj] of Object.entries(intRaw)) {
    for (const p of obj.patterns) {
      ...
      if (re.test(txt)) { arr.push(intent); break; }
    }
  }
  return arr;
}
...
function loadAnsForKW(k, remote, base) {
  ...
  if (!e) {
    return [{ answers: [`عذرًا، لا تتوفر إجابة مفصّلة عن «${k}».`], proof: [] }];
  }
  const file = path.join(base, e.file);
  if (!fs.existsSync(file)) {
    return [{ answers: [`عذرًا، تعذّر العثور على ملف الإجابات لـ «${k}».`], proof: [] }];
  }
...
function bestAnswer(arr, intent, type, cond, place) {
  ...
  for (const e of arr) {
    ...
    if (place) {
      const ok = Array.isArray(e.place) ? e.place.includes(place) : e.place === place;
      if (ok) s++;
    }
...
function handleMissingQ(question, matchedKeyword = "", mayIntent, base = "./data") {
...
console.log(`معالجة سؤال مفقود: ${question} (matchedKeyword: ${matchedKeyword})`);
  ...
  if (matches.length) {
    ...
    if (priority(best) >= 2000) {   // keyword أو variant مؤكَّد
      ctx.keyword = best.keyword.startsWith("ال") ? best.keyword : `ال${best.keyword}`;
    }
    ctx.type      ??= best.type;
  ...
  if (!hasKW && intents.length === 1 && question.trim().split(/\s+/).length <= 1) {
    const i = intents[0];
    return {
      ask: "keyword",
  ...
  if (uniqKws.length > 1 && !matchedKeyword) {
    ...
    return {
      ask: "keyword",
      message  : `سؤالك عن «${question}» يحتمل: ${combos.join(" أم ")}. حدّد المطلوب.`,
  ...
  if (!ctx.intent) {
    const intentsAvailable = [...new Set(answersAll.map((a) => a.intent).filter(Boolean))];
    if (intentsAvailable.length > 1) {
      partialContext = ctx;
      return {
        ask: "intent",
        message  : `لدي أكثر من إجابة ممكنة بخصوص ${ctx.keyword} (${intentsAvailable.join("، ")}). حدِّد المطلوب.`,
        available: { keyword: true, intent: false, context: true },
        context  : ctx
      };
    }
  ...
  return {
    ...
    condition : ctx.condition || null,
    place     : ctx.place     || null,
    answer    : "نأسف لعدم توفر الإجابة على هذا السؤال حالياً، يرجى المحاولة لاحقاً.",
    score     : 0.6
  };
}
